...
def silhouette_score(data: DataFrame, distance):
    ...
    :param data: A Pandas DataFrame that must contain exactly a column named "cluster", that indicates the id of the instance's cluster.
    :param distance: The distance function used.
    :return: A number between -1 and 1 that represents the score of the clustering.
    """
...
def a_index(cluster: DataFrame, distance):
    ...
    :param cluster: The cluster in which the a_index is calculated.
    :param distance: The distance function used.
    :return: The cluster passed in input, but with the column 'a_index' added.
    """
...
def b_index(in_cluster: DataFrame, out_cluster: DataFrame, distance):
    """
    Adds the column 'b_index' to the cluster passed in input. The b_index is the minimum distance between an element of the in_cluster
    and every other element in the out_cluster.
    :param in_cluster: The cluster in which the b_index is calculated.
    :param out_cluster: The dataframe that contains each element that does not belong to the in_cluster
    :param distance: The distance function used.
    :return: The cluster passed in input, but with the column 'b_index' added.
    """
    in_cluster['b_index'] = in_cluster['cluster'].copy()
    for i in range(in_cluster.shape[0]):
        result = 0
        for j in range(out_cluster.shape[0]):
            result = min(result, distance(in_cluster.iloc[i], out_cluster.iloc[j]) / out_cluster.shape[0])
        in_cluster.iloc[i, in_cluster.columns.get_loc("b_index")] = result
    return in_cluster
